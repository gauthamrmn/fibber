...
#let deposit_helper(layer-height, layer, width, height, position, color) = {
  draw.rect(
...
#let etch_helper(layer-height, bottom, top, width, position, color) = {
    draw.rect(
    ...
    ( 
     layer-increment,
      for p in pattern(device-anchors) {
        deposit_helper(layer-height, layer, p.at(1) - p.at(0), height, p.at(0), materials.at(material))
      }
    )
  }
}
...
#let etch(height: 1, pattern: ((left, middle, right)) => ((left, right),), start-layer: none) = {
  (layer-height, current-layer, materials, device-anchors, background-color) => {
    ...
    (
      layer-increment,
      for p in pattern(device-anchors) {
        etch_helper(layer-height, layer - height, layer, p.at(1) - p.at(0), p.at(0), background-color)
      }
    )
  }
}
